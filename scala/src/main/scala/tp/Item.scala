...
case class Item(restricciones: Seq[Restriccion] = Seq(), modificaciones: Heroe => Stats = (heroe) => heroe.statsBase) {
...
object Restricciones {
  ...
  def trabajosValidos(trabajos: Seq[Trabajo]): Restriccion = (heroe: Heroe) => trabajos.contains(heroe.trabajo.get)
...
object cascoVikingo extends Cabeza(item = Item(restricciones = Seq(Restricciones.statMinimo(Fuerza, 30)),
